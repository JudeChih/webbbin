...
class ProjectDataRepository {
    ...
    public function getAllData($arraydata){
        ...
        if($arraydata['limit'] != 'all'){
            return $string->orderBy($arraydata['orderBy'],'DESC')->paginate($arraydata['limit']);
        }else{
    ...
    public function getFiveLatestAdaptiveData(){
        return ProjectData::where('pd_rwd',0)->where('isflag',1)->orderBy('create_date','DESC')->take(5)->get();
    }
