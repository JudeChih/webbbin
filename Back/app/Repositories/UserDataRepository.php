...
class UserDataRepository {
	...
	public function getAllData($arraydata){
    	...
    	if(CommonTools::checkArrayValue($arraydata, "ud_account")){
    		$string->where('ud_account','like', '%'.$arraydata['ud_account'].'%')->orWhere('ud_name','like', '%'.$arraydata['ud_account'].'%');
    	}
    	return $string->orderBy('create_date','DESC')->paginate(15);
	}
	...
	public function getAllUserLastLogin($arraydata){
		if($arraydata['limit'] != 'all'){
			return UserData::where('isflag',1)->where('ud_last_login',"!=",null)->orderBy('ud_last_login','DESC')->paginate($arraydata['limit']);
		}else{
	...
	public function create($arraydata){
		try {
			...
			// 新增使用者
			return UserData::insert($savedata);
		} catch (\Exception $e) {
			CommonTools::writeErrorLogByException($e);
			return false;
		}
