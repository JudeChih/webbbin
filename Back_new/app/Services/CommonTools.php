...
class CommonTools {
    ...
    public static function id_card($cardid){
        ...
        // CommonTools::writeErrorLogByMessage('身份字號：'.$cardid);
        // CommonTools::writeErrorLogByMessage($msg);
        return $msg;
    }

    /**
     * 將Json格式的字串 轉換為 PHP Array
     * @param json $inputstring WebAPI接收到的「JSON」格式字串
     * @return array PHP陣列
     */
     ...
     * @param array $arraydata   要檢查的陣列
     * @param string $keyname    要檢查的參數名稱
     * @param string $maxlength 最大長度限制，若輸入「0」則為不限制
     * @param boolean $canempty 是否可為「空值」
     * @param boolean $canspace 是否可包含「空白」
     ...
     * @param string $functionname 執行的功能名稱
     * @param string $input 接收到的值
     * @param string $result 回傳的值
     * @param string $messagecode 訊息代碼
     * @return boolean 執行結果
     */
